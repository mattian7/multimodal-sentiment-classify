...
class SELF_MM(nn.Module):
    ...
    def forward(self, image, text):
        if self.args.mode == 'all':
            image_hidden_state = self.image_model(**image).pooler_output.squeeze()
            ...
            fusion_h = self.fusion_dropout(image_text_hidden_state)
            fusion_h = F.relu(self.fusion_layer_1(fusion_h), inplace=False)
            ...
            out = torch.mean(torch.stack([output_image, output_text, output_fusion], dim=1), dim=1)

            res = {
                'fusion_output': output_fusion,
